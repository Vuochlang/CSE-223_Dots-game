...
public class GamePanel extends JPanel {
  ...
  public void saveParent(MainFrame mainParent)
  {	parent=mainParent;
  }
  ...
  {	start=false;
    ...
    {	player1=parent.player1.getText().toUpperCase();
      player2=parent.player2.getText().toUpperCase();
      if(player1.equals(player2))//if they entered the same name, a message will print out and the game wont start until they change the name
      {	parent.txArea1.setForeground(Color.red);
        parent.txArea1.setText("Players' name cannot be the same...");
        sameName=true;
        return;
      }
    }
  ...
  {	left=right=top=bottom=false;
    ...
    {	//set color to the box according to the box.initial
      if(box[i][j].initial==Character.toUpperCase(player1.charAt(0)))
      {	g.setColor(color1);
        g.fillRect(box[i][j].topLeftX,box[i][j].topLeftY,50,50);
      }
      if(box[i][j].initial==Character.toUpperCase(player2.charAt(0)))
      {	g.setColor(color2);
        g.fillRect(box[i][j].topLeftX,box[i][j].topLeftY,50,50);
      }
    }
      ...
      g.fillOval(box[i][j].topLeftX-2, box[i][j].topLeftY-2, dotSize, dotSize);
      if(i==7&&j==7)	g.fillOval(box[i][j].bottomRightX-2, box[i][j].bottomRightY-2, dotSize, dotSize);
      if(j==7)	g.fillOval(box[i][j].topRightX-2, box[i][j].topRightY-2, dotSize, dotSize);
      if(i==7)	g.fillOval(box[i][j].bottomLeftX-2, box[i][j].bottomLeftY-2, dotSize, dotSize);
      ...
      if(start)
      {	if(box[i][j].initial==Character.toUpperCase(player1.charAt(0)))	++count1;
        if(box[i][j].initial==Character.toUpperCase(player2.charAt(0)))	++count2;
      }
    ...
    {	if(!darkMode)	parent.txArea1.setForeground(Color.black);
      ...
      if(first)
      {	parent.txArea1.setText("This is "+player1.charAt(0)+"'s turn");
        first=false;
      }
  ...
  public void click(int x, int y)	//this will call when the player clicks in the JPanel in the Dots Area
  {	xPosition=x;
    yPosition=y;
  ...
  {	if(!start)	return("player1 : "+count1+"	player2 : "+count2);
    return(player1.toUpperCase()+" : "+count1+"		"+player2.toUpperCase()+": "+count2);
  }

  public void changeMode(int count) //this will change the mode of the game to black
  {	Color bgColor=Color.white;
    Color txColor=Color.black;
    darkMode=false;

    if(count%2!=0)//if darkMode
    {	bgColor = new Color(32,32,32);
      txColor=Color.white;
      darkMode=true;
    }

    parent.contentPane.setBackground(bgColor);
    parent.lblNewLabel1.setForeground(txColor);
    parent.lblNewLabel2.setForeground(txColor);
    parent.lblScore.setForeground(txColor);
    parent.player1.setBackground(bgColor);
    parent.player1.setForeground(txColor);
    parent.player2.setBackground(bgColor);
    parent.player2.setForeground(txColor);
    parent.score.setBackground(bgColor);
    parent.score.setForeground(txColor);
    parent.rdbtnNewRadioButton.setBackground(bgColor);
    parent.rdbtnNewRadioButton.setForeground(txColor);
    parent.panel.setBackground(bgColor);
    parent.panel.setBorder(new LineBorder(txColor));
    parent.txArea1.setBackground(bgColor);
    parent.lblInstruction.setBackground(bgColor);
    parent.lblInstruction.setForeground(txColor);
    parent.txtrInstructionPressButton.setBackground(bgColor);
    parent.txtrInstructionPressButton.setForeground(txColor);
  }
